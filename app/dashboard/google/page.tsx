...
const STORAGE_KEY = 'apex_google_columns';
...
export default function GooglePage() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from('campaigns').select('*').eq('platform', 'google').order('spend', { ascending: false });
      setCampaigns(data || []);
      setLoading(false);
      const { data: conn } = await supabase.from('ad_connections').select('last_synced_at').eq('platform', 'google').eq('status', 'active').limit(1).single();
  ...
  async function syncAds(preset?: string, since?: string, until?: string) {
    ...
    try {
      ...
      if (data.error) {
        ...
        const googleCampaigns = (data.campaigns || []).filter((c: Campaign) => c.platform === 'google');
        setCampaigns(googleCampaigns);
        setLastSync(data.synced_at);
        setSyncMsg(`Synced ${googleCampaigns.length} Google campaigns`);
      }
    } catch (e: any) {
  ...
  const kpis = [
    { label: 'Google Spend', value: `$${totalSpend.toLocaleString()}` },
    ...
    <div>
      ...
      {!loading && campaigns.length === 0 && (
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '40px 24px', textAlign: 'center' }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}>No Google campaigns</h3>
          <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 16 }}>Connect your Google Ads account in Settings to start syncing campaigns.</p>
          <a href="/dashboard/settings" style={{ display: 'inline-block', padding: '10px 20px', background: 'var(--accent)', color: '#fff', borderRadius: 10, fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>Connect Google</a>
        </div>
      )}
      ...
      {loading && <div style={{ textAlign: 'center', padding: 40, color: 'var(--muted)' }}>Loading Google campaigns...</div>}
    </div>
  );
}
