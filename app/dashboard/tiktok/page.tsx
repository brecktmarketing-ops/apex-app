...
const STORAGE_KEY = 'apex_tiktok_columns';
...
export default function TikTokPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setCampaigns(data || []);
      setLoading(false);
      const { data: conn } = await supabase.from('ad_connections').select('last_synced_at').eq('platform', 'tiktok').eq('status', 'active').limit(1).single();
  ...
  async function syncAds(preset?: string, since?: string, until?: string) {
    ...
    try {
      ...
      const res = await fetch(`/api/ads/tiktok?${params}`);
      const data = await res.json();
      if (data.error) {
        setSyncMsg(data.error);
      } else {
        const tiktokCampaigns = (data.campaigns || []).filter((c: Campaign) => c.platform === 'tiktok');
        setCampaigns(tiktokCampaigns);
        setLastSync(data.synced_at);
        setSyncMsg(`Synced ${tiktokCampaigns.length} TikTok campaigns`);
      }
    } catch (e: any) {
  ...
  const kpis = [
    { label: 'TikTok Spend', value: `$${totalSpend.toLocaleString()}` },
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={() => syncAds()} disabled={syncing} style={{ padding: '8px 18px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: syncing ? 0.6 : 1 }}>
            {syncing ? 'Syncing...' : 'Sync TikTok Ads'}
          </button>
      ...
      {!loading && campaigns.length === 0 && (
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '40px 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 16 }}>Connect your TikTok Ads account in Settings to start syncing campaigns.</p>
      ...
      {loading && <div style={{ textAlign: 'center', padding: 40, color: 'var(--muted)' }}>Loading TikTok campaigns...</div>}
    </div>
  );
}
